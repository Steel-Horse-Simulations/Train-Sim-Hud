...
EXPORT_TABLES = ["train_classes", "loco_groups", "loco_subclasses",
                 "class_families", "operators", "operator_liveries",
                 "train_class_aliases"]


def export_everything():
    """A COMPLETE dump of the Known Trains database - every table, every row,
    every column.

    Deliberately raw tables rather than the resolved view. The old download
    button saved /api/known_trains/list, which is the DRIVEN-ONLY, resolved
    view: it silently omitted every catalog row with times_seen = 0, every
    hidden row, and all of the variants, subclasses, families, operators,
    liveries and aliases. Restoring from that after a wipe would have lost
    most of the data while looking like a successful backup.

    Generated by walking sqlite_master, so a table added later is included
    ...
    try:
        ...
        return {
            "format": "tsw_hud_known_trains_backup",
            "format_version": 1,
...
def import_everything(payload, replace=False):
    """Restores a dump produced by export_everything().

    replace=False merges, skipping any row whose id already exists, so a
    restore cannot quietly overwrite work done since the backup.
    replace=True clears each table first.
    """
    if not isinstance(payload, dict) or "tables" not in payload:
        raise ValueError("not a Known Trains backup file")
    ...
    try:
        ...
        # Parents before children. Without this, tables restore in
        # alphabetical order, which puts operator_liveries BEFORE operators -
        # every livery then fails its foreign key and is dropped. That is
        # exactly what happened on the first restore test: trains and
        # operators came back, liveries silently did not.
        order = ["operators", "class_families", "loco_groups",
                 "loco_subclasses", "operator_liveries", "train_classes",
                 "train_class_aliases"]
        ordered = [t for t in order if t in tables]
        ordered += [t for t in tables if t not in order]

        for name in ordered:
            rows = tables.get(name)
            if name not in existing or not isinstance(rows, list):
                continue
            ...
            for row in rows:
                ...
                names_sql = ", ".join(f'"{k}"' for k in use)
                try:
                    conn.execute(
                        f'INSERT OR {"REPLACE" if replace else "IGNORE"} INTO "{name}" '
                        f'({names_sql}) VALUES ({placeholders})', list(use.values()))
                    a += 1
                except Exception as e:
                    s_ += 1
                    # Record WHY. A restore that quietly drops rows is worse
                    # than one that fails outright, because it looks like it
                    # worked.
                    errors.setdefault(name, str(e))
            added[name], skipped[name] = a, s_
        conn.commit()
